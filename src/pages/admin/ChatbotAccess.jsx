...
import { FaRobot, FaSearch, FaSync, FaCheck, FaTimes } from "react-icons/fa";
import { useChatbotAccess, useUpdateChatbotAccess } from "../../hooks/queries";
import { useQueryClient } from "@tanstack/react-query";

export default function ChatbotAccess() {
    const queryClient = useQueryClient();
    const [query, setQuery] = useState("");
    const [selected, setSelected] = useState(new Set());

    const { data, isLoading, isError, error, isFetching } = useChatbotAccess();
    ...
    const enabledCount = orgs.filter((o) => o.chatbot_access_enabled).length;
    ...
    async function toggleSingle(org) {
        try {
            ...
            toast.success(`${org.name} — chatbot access ${!org.chatbot_access_enabled ? "enabled" : "disabled"}`);
        } catch (err) {
    ...
    async function bulkUpdate(enabled) {
        ...
        const results = await Promise.allSettled(ids.map((id) => updateChatbot.mutateAsync({ id, enabled })));
        const failed = results.filter((r) => r.status === "rejected").length;
        const succeeded = ids.length - failed;

        if (succeeded > 0) toast.success(`${enabled ? "Enabled" : "Disabled"} chatbot access for ${succeeded} organization(s)`);
        ...
        <div className="space-y-6">
                ...
                <div>
                    <div className="flex items-center gap-2">
                        ...
                        <h1 className="text-2xl font-bold tracking-wide text-(--text-primary) uppercase">
                            Chatbot Access Management
                        </h1>
                    </div>
                    <p className="text-(--text-secondary) mt-1">Manage chatbot access for Level 2 organizations in bulk.</p>
                </div>
                <button
            ...
            <section className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div className="bg-(--bg-card) border border-emerald-300 dark:border-emerald-800 rounded-xl shadow-sm p-5">
                    <p className="text-sm text-emerald-600 dark:text-emerald-400">Chatbot Enabled</p>
                ...
                <div className="bg-(--bg-card) border border-red-300 dark:border-red-800 rounded-xl shadow-sm p-5">
                    <p className="text-sm text-red-600 dark:text-red-400">Chatbot Disabled</p>
                                    ...
                                    {filtered.map((org) => (
                                        <tr key={org.id} className="border-b border-(--border-color) hover:bg-(--bg-subtle) transition text-(--text-primary)">
                                            ...
                                            <td className="px-4 py-4">
                                                ...
                                                >
                                                    {org.chatbot_access_enabled ? <FaCheck size={10} /> : <FaTimes size={10} />}
                                                    {org.chatbot_access_enabled ? "Enabled" : "Disabled"}
                                                </button>
                                            </td>
                                        </tr>
                                    ))}
